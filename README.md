...
```
203.0.113.5:42298:uEiAxk...9Qw
[2001:db8::7]:42298:uEiAxk...9Qw
```
...
`--network=host` is needed at build time for fetching crates, and at run time for reaching Tor directory authorities. Use `-p 42298:42298/udp` instead if your Docker bridge has working outbound connectivity — the KPS port is UDP; both QUIC and WebRTC ride it. Note that WebRTC over IPv6 requires the socket to be bound to the wildcard address the clients actually reach (the default `:42298` dual-stack bind handles this).
